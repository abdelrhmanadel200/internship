...
const PricingCard = ({ plan, price, attempts }:PricingCardProps )=> (
  <div className={`w-96 h-[450px] rounded-lg border-2 border-primary bg-white flex flex-col items-center justify-evenly transition duration-300 ease-in-out text-primary hover:bg-primary  hover:text-white cursor-pointer`}>
    <div className="text-6xl ">{plan}</div>
    <div className="text-5xl ">{price}</div>
    <div className="text-3xl text-black">{attempts}</div>
    <button className="w-40 rounded-lg text-xl bg-primary text-white py-2 transition duration-300 hover:bg-teal-600">
      Get it
    </button>
  </div>
);
