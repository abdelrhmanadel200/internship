  ...
  return (
    <div className={`py-5 ${inter.className} bg-primary`}>
      ...
      {isOpen && (
        <div
          ref={menuRef}
          className="absolute right-0 mt-2 w-100% bg-white shadow-lg rounded-lg z-20 transition-transform transform origin-top-right scale-100 mx-4"
        >
          <div className="flex flex-col lg:hidden">
            {navItems.map(({ name, path }) => (
              <Link
                key={name}
                href={path}
                className=" text-lg font-semibold hover:bg-[#f0f0f0] transition duration-200 p-3 rounded-lg mx-5"
